...
####### while set to false, the game stays frozen.##################
@export var game_is_running = false
####################################################################
...
func game_over():
	if score > high_score:
		high_score = score
		new_high_score = true
	get_tree().call_deferred("change_scene_to_file","res://scenes/menu_scenes/game_over_menu.tscn")
